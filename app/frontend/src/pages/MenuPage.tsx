...
export default function MenuPage() {
  ...
  if (loading) return <div className="animate-pulse text-text-muted">Loading menu...</div>;
  ...
  return (
    <div>
      ...
      {/* Categories */}
      <div className="mb-6">
        <h2 className="text-sm font-semibold text-text-muted uppercase tracking-wide mb-3">Categories</h2>
        <div className="flex flex-wrap gap-2">
          {categories.map((cat) => (
            <span key={cat.id} className="px-3 py-1.5 bg-surface border border-divider rounded-lg text-sm font-medium text-text-secondary">
              {cat.name}
            </span>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {items.map((item) => (
          <div key={item.id} className="bg-surface border border-divider rounded-xl p-4 relative">
            <div className="flex items-start justify-between mb-2">
              <div className="flex items-center gap-2">
                <span className={`w-5 h-5 rounded-sm flex items-center justify-center text-xs font-bold ${item.is_veg ? 'bg-status-completed text-white' : 'bg-status-destructive text-white'}`}>
                  {item.is_veg ? 'V' : 'N'}
                </span>
                <h3 className="font-semibold text-text-primary">{item.name}</h3>
              </div>
              <div className="flex gap-1">
                <button onClick={() => openEdit(item)} className="p-1.5 hover:bg-cream-200 rounded-lg transition">
                  <Pencil className="w-4 h-4 text-text-muted" />
                </button>
                <button onClick={() => handleDeleteItem(item.id)} className="p-1.5 hover:bg-status-destructive-soft rounded-lg transition">
            ...
            <div className="flex items-center justify-between">
              <span className="text-lg font-bold text-text-primary">₹{Number(item.price).toFixed(0)}</span>
              <button
                onClick={() => toggleAvailability(item)}
                className={`px-3 py-1 rounded-full text-xs font-medium transition ${
                  item.is_available
                    ? 'bg-status-completed-soft text-status-completed hover:bg-status-completed/20'
            ...
            {item.category_name && (
              <span className="inline-block mt-2 px-2 py-0.5 bg-cream-200 text-text-secondary text-xs rounded-md">
                {item.category_name}
              </span>
            )}
          </div>
        ...
        <div className="fixed inset-0 bg-overlay flex items-center justify-center z-50 p-4">
          <div className="bg-surface rounded-2xl w-full max-w-md p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowItemModal(false)} className="p-1 hover:bg-cream-200 rounded-lg">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="space-y-3">
              ...
              <div className="flex gap-3">
                ...
                <input
                  ...
                  className="w-24 px-3 py-2 border border-divider rounded-lg text-sm focus:ring-2 focus:ring-action outline-none"
                />
              </div>
              <select
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowItemModal(false)} className="flex-1 py-2 border border-divider rounded-lg text-sm font-medium hover:bg-cream-50 transition">
                Cancel
              </button>
              <button onClick={handleSaveItem} className="flex-1 py-2 bg-action text-white rounded-lg text-sm font-medium hover:bg-action-hover transition">
